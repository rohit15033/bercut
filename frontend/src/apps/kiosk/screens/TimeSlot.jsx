...
export default function TimeSlot({ barber, branchId, serviceIds, setServiceIds, services, menuItems, slot, setSlot, selectedExtras, setSelectedExtras, onNext, onBack }) {
  ...
  const showNowPicker = !canNow && nowWindow?.freeNow && nowWindow?.windowMin > 0 && totalDur > nowWindow.windowMin
  ...
  const ItemCard = ({ item, compact = false }) => {
    ...
    return (
      <div onClick={() => !oos && toggleExtra(itemId)}
        ...
        {oos && <div style={{ position:'absolute', top:6, right:6, background:C.danger, color:C.white, fontSize:'clamp(8px,1vw,10px)', fontWeight:700, padding:'1px 7px', borderRadius:4 }}>HABIS</div>}
          ...
          {loadingSlots
            ...
            : (
              <div className="slot-grid">
                ...
                {slots.map((s, i) => (
                  <button key={s} onClick={() => pickGridSlot(s)}
                ...
                {!loadingSlots && slots.length === 0 && (
                  <div style={{ color:C.muted, fontSize:'clamp(12px,1.4vw,14px)', padding:'16px 0' }}>No available slots today. Try a different barber.</div>
                )}
              </div>
            )
          }
...
function NowPickerModal({ windowMin, nextSlot, originalServices = [], allServices = [], onConfirm, onClose }) {
  ...
  const [changingId, setChangingId]           = useState(null) // service.id being swapped, null = checklist view
  ...
  const overflowMin = checkedDur - windowMin
  const fits        = checkedDur <= windowMin && checkedIds.size > 0
  const pct         = Math.round((checkedDur / windowMin) * 100)
  ...
  // Services available for swap picker: fit in window, not already in current list (except the one being swapped)
  const pickerSvcs = allServices
    .filter(s => getDur(s) <= windowMin && (s.id === changingId || !currentServices.some(c => c.id === s.id)))
    .map(normSvc)
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', zIndex:800, display:'flex', alignItems:'center', justifyContent:'center', padding:'clamp(16px,3vw,32px)' }}
      ...
      <div className="si"
        style={{ background:C.white, borderRadius:20, width:'100%', maxWidth: changingId ? 'clamp(400px,64vw,740px)' : 'clamp(360px,44vw,520px)', maxHeight:'90vh', display:'flex', flexDirection:'column', overflow:'hidden', transition:'max-width 0.2s' }}
        ...
        <div style={{ background:C.topBg, padding:'clamp(14px,1.8vw,20px) clamp(20px,2.6vw,28px)', flexShrink:0 }}>
          ...
          {/* Progress bar — checklist view only */}
          {!changingId && (
            <>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', marginTop:6 }}>
                <div style={{ fontSize:'clamp(10px,1.1vw,12px)', color: fits ? '#888' : C.danger, fontWeight: fits ? 400 : 700 }}>
                  {checkedDur} / {windowMin} min
                </div>
                <div style={{ fontSize:'clamp(10px,1.1vw,12px)', color: fits ? '#888' : C.danger }}>
                  {fits ? `${windowMin - checkedDur} min free` : `−${overflowMin} min to go`}
                </div>
              </div>
            </>
          )}
              ...
              {currentServices.map(s => {
                ...
                return (
                  <div key={s.id}
                    ...
                    {/* Change button — does not propagate to card toggle */}
                    <button
          ...
          {changingId
            ...
            : (
              <>
                ...
                <button onClick={() => onConfirm([...checkedIds])} disabled={!fits} className="btnP" style={{ flex:1 }}>
